package com.xoomworks.attraqt.utilities;

public class Colors {
    //COMMONS
    public static String SECTION_HEADER = "rgb(224, 224, 224)";
    public static String COLOR_REPORT_HEADER = "rgb(7, 62, 89)";
    public static String SEARCH_TABLE_HEADER_COLOR = "rgb(166, 166, 166)";
    public static String TABLE_CONTENS_COLOR = "rgb(166, 166, 166)";
    ///////////////////////////////////////////////////////////////////////////////////////
    public static String COLOR_MISSPELLED_SEARCH_TERMS_FONT_COLOR = "rgb(63, 105, 126)";
    //PRODUCT PERFORMANCE COLORS
    public static String VIEWS_HEADER = "rgb(242, 242, 242)";
    public static String ADDS_TO_BASKET_HEADER = "rgba(247, 146, 30, 0.184)";
    public static String UNIQUE_PURCHASES_HEADER = "rgba(102, 45, 145, 0.184)";
    //FACETS PERFORMANCE
    public static String FP_SECTION_HEADER = "rgb(224, 224, 224)";
    public static String TABLE_HEADER = "rgb(166, 166, 166)";
    public static String FP_TABLE_HEADER_TEXT_COLOR = "rgb(63, 105, 126)";
    public static String FP_REPORT_HEADER = "rgb(7, 62, 89)";
    //CHECK COLORS CSS
    public static String CSS_BACKGROUND_COLOR = "background-color";
    public static String CSS_BORDER_BOTTOM_COLOR = "border-bottom-color";
    public static String CSS_COLOR = "color";
}

